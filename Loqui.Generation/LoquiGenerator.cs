...
public class LoquiGenerator
{
    ...
    public IEnumerable<ObjectGeneration> ObjectGenerations => ObjectGenerationsByDir.Values.SelectMany((v) => v);
    ...
    public void AddTypicalTypeAssociations()
    {
        ...
        AddTypeAssociation<DateOnlyType>("DateOnly");
        ...
        where Replacement : TypeGeneration
    {
        ...
        var matching = nameToTypeDict.Where((kv) => kv.Value.Equals(t)).ToList();
        if (!matching.Any())
        {
    ...
    {
        await Task.WhenAll(_protocols.Values.Select((p) => p.LoadInitialObjects()));


        await Task.WhenAll(GenerationModules.Select((m) => m.Modify(this)));
        ...
        await Task.WhenAll(GenerationModules.Select((m) => m.FinalizeGeneration(_protocols.Values)));
    ...
    public bool TryGetMatchingObjectGeneration(FilePath file, out ObjectGeneration objGen)
    {
        if (file.Directory is {} dir 
            && ObjectGenerationsByDir.TryGetValue(dir, out List<ObjectGeneration> objs))
        {
