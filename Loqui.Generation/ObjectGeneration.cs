...
namespace Loqui.Generation
{
    public abstract class ObjectGeneration
    {
        ...
        public async Task Generate()
        {
            ...
            using (new NamespaceWrapper(fg, Namespace, fileScoped: false))
            {
                if (GenerateClass)
                {
            ...
            using (new NamespaceWrapper(fg, Namespace, fileScoped: false))
            {
                fg.AppendLine($"public partial class {ObjectName}");
                using (new BraceWrapper(fg))
                {
                    await GenerateRouting(fg, getterOnly: false);
                }
            }
            fg.AppendLine();
        ...
        protected virtual async Task GenerateMixIns(FileGeneration fg)
        {
            ...
            {
                using (var args = new ClassWrapper(fg, $"{MixInClassName}"))
                {
                    args.Static = true;
                    args.Partial = true;
                }
        ...
        protected virtual async Task GenerateSetterInterface(FileGeneration fg)
        {
            ...
            {
                // Internal Interface
                using (var args = new ClassWrapper(fg, Interface(internalInterface: true)))
                {
                    args.Type = ClassWrapper.ObjectType.@interface;
                    args.Partial = true;
        ...
        protected virtual async Task GenerateGetterInterface(FileGeneration fg)
        {
            ...
            using (new BraceWrapper(fg))
            {
                if (IsTopClass)
                {
                    ...
                    fg.AppendLine($"object Common{CommonNameAdditions(interfaceType)}Instance({string.Join(", ", Generics.Select((_, i) => $"Type type{i}"))});");
                    fg.AppendLine("[System.ComponentModel.EditorBrowsable(System.ComponentModel.EditorBrowsableState.Never)]");
                    fg.AppendLine($"object? Common{CommonNameAdditions(LoquiInterfaceType.ISetter, MaskType.Normal)}Instance({string.Join(", ", Generics.Select((_, i) => $"Type type{i}"))});");
                    if (GenerateComplexCopySystems)
                    {
                        fg.AppendLine("[System.ComponentModel.EditorBrowsable(System.ComponentModel.EditorBrowsableState.Never)]");
                    ...
                    fg.AppendLine("[System.ComponentModel.EditorBrowsable(System.ComponentModel.EditorBrowsableState.Never)]");
                    fg.AppendLine($"object Common{CommonNameAdditions(LoquiInterfaceType.ISetter, MaskType.Normal, MaskType.Translation)}Instance();");
                }
                fg.AppendLine($"static{NewOverride()}{nameof(ILoquiRegistration)} StaticRegistration => {RegistrationName}.Instance;");
                foreach (var field in IterateFields())
                {
                    if (!field.GenerateInterfaceMembers) continue;
        ...
        protected void GenerateEnumIndex(FileGeneration fg)
        {
            ...
            {
                fg.AppendLine($"internal enum {FieldIndexName}");
                using (new BraceWrapper(fg))
                {
        ...
        protected async Task GenerateRegistration(FileGeneration fg)
        {
            ...
            {
                using (var args = new ClassWrapper(fg, RegistrationName))
                {
                ...
                using (new BraceWrapper(fg))
                {
                    fg.AppendLine($"public static readonly {RegistrationName} Instance = new {RegistrationName}();");
                    fg.AppendLine();

                    fg.AppendLine($"public static ProtocolKey ProtocolKey => {ProtocolDefinitionName}.ProtocolKey;");
                    fg.AppendLine();

                    fg.AppendLine($"public static readonly ObjectKey ObjectKey = new ObjectKey(");
                    using (new DepthWrapper(fg))
                    {
                        fg.AppendLine($"protocolKey: {ProtocolDefinitionName}.ProtocolKey,");
                        fg.AppendLine($"msgID: {ID},");
                        fg.AppendLine($"version: {Version});");
                    }
                    fg.AppendLine();

                    fg.AppendLine($"public const string GUID = \"{GUID}\";");
                    fg.AppendLine();

                    fg.AppendLine($"public const ushort AdditionalFieldCount = {IterateFields().Count()};");
                    fg.AppendLine();

                    fg.AppendLine($"public const ushort FieldCount = {TotalFieldCount};");
                    fg.AppendLine();

                    fg.AppendLine($"public static readonly Type MaskType = typeof({GetMaskString("")});");
                    fg.AppendLine();

                    fg.AppendLine($"public static readonly Type ErrorMaskType = typeof({Mask_Unspecified(MaskType.Error)});");
                    fg.AppendLine();

                    fg.AppendLine($"public static readonly Type ClassType = typeof({Name}{EmptyGenerics()});");
                    fg.AppendLine();

                    fg.AppendLine($"public static readonly Type GetterType = typeof({InterfaceNoGenerics(getter: true)}{EmptyGenerics()});");
                    fg.AppendLine();

                    fg.AppendLine($"public static readonly Type? InternalGetterType = {(HasInternalGetInterface ? $"typeof({InterfaceNoGenerics(getter: true, internalInterface: true)}{EmptyGenerics()})" : "null")};");
                    fg.AppendLine();

                    fg.AppendLine($"public static readonly Type SetterType = typeof({InterfaceNoGenerics()}{EmptyGenerics()});");
                    fg.AppendLine();

                    fg.AppendLine($"public static readonly Type? InternalSetterType = {(HasInternalSetInterface ? $"typeof({InterfaceNoGenerics(internalInterface: true)}{EmptyGenerics()})" : "null")};");
                    fg.AppendLine();

                    fg.AppendLine($"public const string FullName = \"{FullName}\";");
                    fg.AppendLine();

                    fg.AppendLine($"public const string Name = \"{Name}\";");
                    fg.AppendLine();

                    fg.AppendLine($"public const string Namespace = \"{Namespace}\";");
                    fg.AppendLine();

                    fg.AppendLine($"public const byte GenericCount = {Generics.Count};");
                    fg.AppendLine();

                    fg.AppendLine($"public static readonly Type? GenericRegistrationType = {(Generics.Count > 0 ? $"typeof({RegistrationName}{EmptyGenerics()})" : "null")};");
                    fg.AppendLine();
                ...
                {
                    fg.AppendLine();
                    using (var args = new ClassWrapper(fg, $"{RegistrationName}{GetGenericTypes(MaskType.Normal)}"))
                    {
                        args.Public = PermissionLevel.@internal;
                    ...
                    using (new BraceWrapper(fg))
                    {
                        fg.AppendLine($"public static readonly {RegistrationName}{GetGenericTypes(MaskType.Normal)} GenericInstance = new {RegistrationName}{GetGenericTypes(MaskType.Normal)}();");
                        fg.AppendLine();
        ...
        private async Task GenerateCommonClass(FileGeneration fg, MaskTypeSetCollection activeMaskCollections, LoquiInterfaceType interfaceType, CommonGenerics commonGen, params MaskType[] maskTypes)
        {
            ...
            using (new BraceWrapper(fg))
            {
                fg.AppendLine($"public{NewOverride()}static readonly {className} Instance = new {className}();");
                fg.AppendLine();
        ...
        private void GenerateCommonToString(FileGeneration fg, MaskTypeSet maskTypes)
        {
            ...
            using (new BraceWrapper(fg))
            {
                ...
                using (new BraceWrapper(fg))
                {
                    fg.AppendLine($"fg.AppendLine($\"{Name}{generics} =>\");");
                }
                fg.AppendLine("else");
                using (new BraceWrapper(fg))
                {
                    fg.AppendLine($"fg.AppendLine($\"{{name}} ({Name}{generics}) =>\");");
                }
            ...
            using (new BraceWrapper(fg))
            {
                ...
                using (new BraceWrapper(fg))
                {
                    foreach (var (PublicIndex, InternalIndex, Field) in baseObj.IterateFieldIndices(includeBaseClass: true))
                    {
                        ...
                        using (new DepthWrapper(fg))
                        {
                            fg.AppendLine($"return ({FieldIndexName})((int)index);");
                        }
                    }
        ...
        protected virtual async Task GenerateCopyInExtensions(FileGeneration fg)
        {
            ...
            using (new BraceWrapper(fg))
            {
                ...
                fg.AppendLine($"errorMask = {Mask(MaskType.Error)}.Factory(errorMaskBuilder);");
            }
            fg.AppendLine();
        ...
        public async Task GenerateRouting(FileGeneration fg, bool getterOnly)
        {
            ...
            using (new RegionWrapper(fg, "Common Routing"))
            {
                ...
                fg.AppendLine($"ILoquiRegistration ILoquiObject.Registration => {RegistrationName}.Instance;");
                fg.AppendLine($"public{NewOverride()}static {nameof(ILoquiRegistration)} StaticRegistration => {RegistrationName}.Instance;");
                fg.AppendLine("[DebuggerStepThrough]");
                fg.AppendLine($"protected{FunctionOverride()}object Common{CommonNameAdditions(LoquiInterfaceType.IGetter, MaskType.Normal)}Instance({typeInParams}) => {AddGenericWrap($"{CommonClassRouter(maskSets, LoquiInterfaceType.IGetter, CommonGenerics.Class, MaskType.Normal)}.Instance")};");
                if (!getterOnly)
                {
                    fg.AppendLine("[DebuggerStepThrough]");
                    using (var args = new FunctionWrapper(fg,
                ...
                fg.AppendLine("[DebuggerStepThrough]");
                fg.AppendLine($"protected{FunctionOverride()}object Common{CommonNameAdditions(LoquiInterfaceType.ISetter, MaskType.Normal, MaskType.Translation)}Instance() => {CommonClassRouter(maskSets, LoquiInterfaceType.ISetter, CommonGenerics.Functions, MaskType.Normal, MaskType.Translation)}.Instance;");
                if (IsTopClass)
                {
                    fg.AppendLine("[DebuggerStepThrough]");
                    fg.AppendLine($"object {Interface(getter: true, internalInterface: false)}.Common{CommonNameAdditions(LoquiInterfaceType.IGetter)}Instance({typeInParams}) => this.Common{CommonNameAdditions(LoquiInterfaceType.IGetter)}Instance({typeOutParams});");
                    if (getterOnly)
                    {
                        fg.AppendLine("[DebuggerStepThrough]");
                        fg.AppendLine($"object? {Interface(getter: true, internalInterface: false)}.Common{CommonNameAdditions(LoquiInterfaceType.ISetter)}Instance({typeInParams}) => null;");
                        if (GenerateComplexCopySystems)
                        {
                            fg.AppendLine("[DebuggerStepThrough]");
                            fg.AppendLine($"object {Interface(getter: true, internalInterface: false)}.Common{CommonNameAdditions(LoquiInterfaceType.ISetter, MaskType.Normal, MaskType.Copy)}Instance{GetGenericTypes(MaskType.Copy)}({typeInParams}) => null;");
                        }
                    }
                    else
                    {
                        fg.AppendLine("[DebuggerStepThrough]");
                        fg.AppendLine($"object {Interface(getter: true, internalInterface: false)}.Common{CommonNameAdditions(LoquiInterfaceType.ISetter)}Instance({typeInParams}) => this.Common{CommonNameAdditions(LoquiInterfaceType.ISetter)}Instance({typeOutParams});");
                        if (GenerateComplexCopySystems)
                        {
                            fg.AppendLine("[DebuggerStepThrough]");
                            fg.AppendLine($"object {Interface(getter: true, internalInterface: false)}.Common{CommonNameAdditions(LoquiInterfaceType.ISetter, MaskType.Normal, MaskType.Copy)}Instance{GetGenericTypes(MaskType.Copy)}({typeInParams}) => this.Common{CommonNameAdditions(LoquiInterfaceType.ISetter, MaskType.Normal, MaskType.Copy)}Instance{GetGenericTypes(MaskType.Copy)}({typeOutParams});");
                        }
                    }
                    fg.AppendLine("[DebuggerStepThrough]");
                    fg.AppendLine($"object {Interface(getter: true, internalInterface: false)}.Common{CommonNameAdditions(LoquiInterfaceType.ISetter, MaskType.Normal, MaskType.Translation)}Instance() => this.Common{CommonNameAdditions(LoquiInterfaceType.ISetter, MaskType.Normal, MaskType.Translation)}Instance();");
                }
                fg.AppendLine();
        ...
        public async Task GenerateLoquiReflectionGetterInterface(FileGeneration fg)
        {
            using (new RegionWrapper(fg, "Loqui Getter Interface"))
            {
                ...
                fg.AppendLine($"protected{FunctionOverride()}object GetNthObject(ushort index) => {CommonClassName(LoquiInterfaceType.IGetter)}.GetNthObject{GetGenericTypes(MaskType.Normal)}(index, this);");
                if (IsTopClass)
                {
                ...
                fg.AppendLine($"protected{FunctionOverride()}void UnsetNthObject(ushort index) => {CommonClassName(LoquiInterfaceType.ISetter)}.UnsetNthObject{GetGenericTypes(MaskType.Normal)}(index, this);");
                if (IsTopClass)
                {
        ...
        private void GenerateGetNthObject(FileGeneration fg, MaskTypeSet maskTypes)
        {
            ...
            using (new BraceWrapper(fg))
            {
                fg.AppendLine($"{FieldIndexName} enu = ({FieldIndexName})index;");
        ...
        protected virtual async Task GenerateSetNthObject(FileGeneration fg)
        {
            ...
            fg.AppendLine($"protected{FunctionOverride()}void SetNthObject(ushort index, object obj)");
            using (new BraceWrapper(fg))
            {
                fg.AppendLine($"{FieldIndexName} enu = ({FieldIndexName})index;");
                fg.AppendLine("switch (enu)");
        ...
        {
            using (var args = new FunctionWrapper(fg, $"public static {GetMaskString("bool")} GetEqualsMask{GetGenericTypes(MaskType.Normal)}"))
            {
                args.Wheres.AddRange(GenerateWhereClauses(LoquiInterfaceType.IGetter));
                args.Add($"this {Interface(getter: true, internalInterface: true)} item");
        ...
        {
            if (!maskTypes.Applicable(LoquiInterfaceType.IGetter, CommonGenerics.Class)) return;
            using (var args = new FunctionWrapper(fg, $"public {GetMaskString("bool")} GetEqualsMask"))
            {
                args.Add($"{Interface(getter: true, internalInterface: true)} item");
                args.Add($"{Interface(getter: true, internalInterface: true)} rhs");
                args.Add($"EqualsMaskHelper.Include include = EqualsMaskHelper.Include.All");
            ...
            using (new BraceWrapper(fg))
            {
                fg.AppendLine($"var ret = new {GetMaskString("bool")}(false);");
                using (var args = new ArgsWrapper(fg,
            ...
            params string[] otherParameters)
        {
            ...
            using (new DepthWrapper(fg))
            {
                if (HasLoquiBaseObject)
                {
                    if (common)
                    {
                        fg.AppendLine($"{(ret ? "return " : string.Empty)}{BaseClass.CommonClassName(loquiInterface)}.{functionName}{BaseGenericTypes}({string.Join(", ", indexAccessor.AsEnumerable().And(otherParameters))});");
                    }
                    else
                    {
        ...
        public void GenerateEqualsSection(FileGeneration fg)
        {
            ...
            if (GenerateEquals)
            {
                using (new RegionWrapper(fg, "Equals and Hash")) 
                {
                    ...
                    using (new BraceWrapper(fg))
                    {
                        fg.AppendLine($"if (obj is not {Interface(getter: true, internalInterface: true)} rhs) return false;");
                        fg.AppendLine($"return {CommonClassInstance("this", LoquiInterfaceType.IGetter, CommonGenerics.Class)}.Equals(this, rhs, crystal: null);");
                    }
                    fg.AppendLine();

                    fg.AppendLine($"public bool Equals({Interface(getter: true, internalInterface: true)}? obj)");
                    using (new BraceWrapper(fg))
                    {
                        fg.AppendLine($"return {CommonClassInstance("this", LoquiInterfaceType.IGetter, CommonGenerics.Class)}.Equals(this, obj, crystal: null);");
                    }
                    fg.AppendLine();

                    fg.AppendLine($"public override int GetHashCode() => {CommonClassInstance("this", LoquiInterfaceType.IGetter, CommonGenerics.Class)}.GetHashCode(this);");
                    fg.AppendLine();
                }
            }
        }
        ...
        protected virtual void GenerateEqualsCommon(FileGeneration fg, MaskTypeSet maskTypes)
        {
            ...
            using (new RegionWrapper(fg, "Equals and Hash"))
            {
                ...
                using (new BraceWrapper(fg))
                {
                    ...
                    if (HasLoquiBaseObject)
                    {
                        fg.AppendLine($"if (!base.Equals(({BaseClass.Interface(getter: true, internalInterface: true)})lhs, ({BaseClass.Interface(getter: true, internalInterface: true)})rhs, crystal)) return false;");
                    }
                    foreach (var field in IterateFields())
                    {
                        if (!HasKeyField() || field.KeyField)
        ...
        private async Task GenerateCreateNew(FileGeneration fg)
        {
            ...
            using (new BraceWrapper(fg))
            {
                ...
                {
                    fg.AppendLine($"return new {ObjectName}();");
                }
            }
            fg.AppendLine();
        }
        ...
        private async Task GenerateCreateNewBasicCommon(FileGeneration fg, MaskTypeSet maskTypes)
        {
            ...
            using (new BraceWrapper(fg))
            {
                fg.AppendLine($"return {Name}{GetGenericTypesNickname("_Setter", MaskType.Normal)}.GetNew();");
            }
            fg.AppendLine();
        }
        ...
        public virtual void GenerateCopy(FileGeneration fg, MaskTypeSet maskTypeSet)
        {
            ...
            using (new BraceWrapper(fg))
            {
                if (Abstract)
                {
                    fg.AppendLine($"{ObjectName} ret = ({ObjectName})System.Activator.CreateInstance(item.GetType());");
                }
                else
                {
                    fg.AppendLine($"{ObjectName} ret = GetNew();");
                }
                using (var args = new ArgsWrapper(fg,
        ...
        public virtual void GenerateDeepCopy(FileGeneration fg, MaskTypeSet maskTypeSet)
        {
            ...
            using (new BraceWrapper(fg))
            {
                fg.AppendLine($"{ObjectName} ret = ({ObjectName}){CommonClassInstance("item", LoquiInterfaceType.IGetter, CommonGenerics.Class, MaskType.NormalGetter)}.GetNew{GetGenericTypes(MaskType.Normal)}();");
                using (var args = new ArgsWrapper(fg,
        ...
        protected void GenerateClear(FileGeneration fg)
        {
            ...
            using (new BraceWrapper(fg))
            {
                fg.AppendLine($"{CommonClassInstance("this", LoquiInterfaceType.ISetter, CommonGenerics.Class)}.Clear(this);");
            }
            fg.AppendLine();
        }
        ...
        protected virtual void GenerateGenericCreate(FileGeneration fg)
        {
            if (!Abstract)
            {
                fg.AppendLine($"public{NewOverride()}static {ObjectName} {Loqui.Internal.Constants.CREATE_FUNC_NAME}(IEnumerable<KeyValuePair<ushort, object>> fields)");
                using (new BraceWrapper(fg))
                {
                    fg.AppendLine($"var ret = new {ObjectName}();");
                    fg.AppendLine($"foreach (var pair in fields)");
                    using (new BraceWrapper(fg))
                    {
                        fg.AppendLine($"CopyInInternal_{Name}(ret, pair);");
                    }
                    fg.AppendLine("return ret;");
                }
                fg.AppendLine();
            ...
            fg.AppendLine($"protected{NewOverride()}static void CopyInInternal_{Name}({ObjectName} obj, KeyValuePair<ushort, object> pair)");
            using (new BraceWrapper(fg))
            {
                fg.AppendLine($"if (!EnumExt.TryParse(pair.Key, out {FieldIndexName} enu))");
                using (new BraceWrapper(fg))
                {
                    if (HasLoquiBaseObject)
                    {
                        fg.AppendLine($"CopyInInternal_{BaseClass.Name}(obj, pair);");
                    }
                    else
                    {
            ...
            if (GenerateNthReflections)
            {
                fg.AppendLine($"public static void {Loqui.Internal.Constants.COPYIN_FUNC_NAME}(IEnumerable<KeyValuePair<ushort, object>> fields, {ObjectName} obj)");
                using (new BraceWrapper(fg))
                {
        ...
        private async Task GenerateNonGenericClass(FileGeneration fg)
        {
            ...
            using var ns = new NamespaceWrapper(fg, Namespace, fileScoped: false);
            using (var args = new ClassWrapper(fg, Name))
            {
                args.Static = true;
            }
            using (new BraceWrapper(fg))
