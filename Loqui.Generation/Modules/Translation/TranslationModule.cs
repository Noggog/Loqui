        ...
        where G : TranslationGeneration
    {
        ...
        private async Task GenerateCopyIn(ObjectGeneration obj, FileGeneration fg)
        {
            ...
            using (new RegionWrapper(fg, $"{this.ModuleNickname} Copy In"))
            {
                ...
                using (var args = new FunctionWrapper(fg,
                    $"public virtual void CopyIn{ModuleNickname}{obj.GetGenericTypes(GetMaskTypes(MaskType.Error))}",
                ...
                foreach (var minorAPI in this.MinorAPIs)
                {
                    ...
                    using (var args = new FunctionWrapper(fg,
                        $"public void CopyIn{ModuleNickname}{obj.GetGenericTypes(GetMaskTypes(MaskType.Error))}",
                        wheres: obj.GenericTypeMaskWheres(LoquiInterfaceType.ISetter, maskTypes: GetMaskTypes(MaskType.Error))))
                    {
        ...
        protected virtual async Task TranslationWrite(ObjectGeneration obj, FileGeneration fg)
        {
            ...
            using (var args = new FunctionWrapper(fg,
                $"public{(inheriting ? " virtual" : null)} void Write"))
            {
